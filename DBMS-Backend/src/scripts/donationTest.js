...
(async ()=>{
  try{
    ...
    const donorToken = donRes.accessToken;
    ...
    const reqToken = reqRes.accessToken;

    console.log('Create emergency');
    const emBody = {bloodGroup:'O_POSITIVE', latitude:22.3569, longitude:91.7832, urgency:'HIGH'};
    r = await fetch(base + '/api/emergency', {method:'POST', headers:{'content-type':'application/json','authorization':'Bearer '+reqToken}, body:JSON.stringify(emBody)});
    const emRes = await r.json();
    console.log('emergency create', emRes);
    const requestId = emRes.request.id;

    console.log('Donor accepts donation');
    r = await fetch(base + '/api/donations', {method:'POST', headers:{'content-type':'application/json','authorization':'Bearer '+donorToken}, body:JSON.stringify({requestId})});
    const donationRes = await r.json();
